...
const pattern1 = /<!-- RINGKASAN SECTION -->[\s\S]*?<button type="submit" id="btnAjukanKegiatan" class="btn btn-primary col-md-6" form="myForm">Ajukan Kegiatan<\/button>\s*<\/div>\s*<\/div>/;
const replacement1 = `<div class="d-flex justify-content-evenly mt-5 border-top pt-4">
...
const pattern2 = /<div class="modal fade" id="informasiPerangkatModal"[\s\S]*?<div class="modal-footer">\s*<button type="button" class="btn btn-danger text-white" data-bs-dismiss="modal">Tutup<\/button>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>/;
const replacement2 = `<!-- MODAL RINGKASAN PENGAJUAN -->
...
const pattern3 = /<!-- Modal Lihat Data Fasilitas -->[\s\S]*?<!-- Modal Tambah Peserta -->/;
content = content.replace(pattern3, '<!-- Modal Tambah Peserta -->');
