...
with open('resources/views/user/kegiatan/kegiatan_step2.blade.php', 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Replace Ringkasan Section (lines 182-220 approx)
old_ringkasan = re.search(r'<!-- RINGKASAN SECTION -->.*?<button type="submit" id="btnAjukanKegiatan" class="btn btn-primary col-md-6" form="myForm">Ajukan Kegiatan</button>\s*</div>\s*</div>', content, re.DOTALL)
if old_ringkasan:
    new_buttons = """<div class="d-flex justify-content-evenly mt-5 border-top pt-4">
    ...
    content = content.replace(old_ringkasan.group(0), new_buttons)

# 2. Replace informasiPerangkatModal with new Modal
old_info_modal = re.search(r'<div class="modal fade" id="informasiPerangkatModal".*?<div class="modal-footer">\s*<button type="button" class="btn btn-danger text-white" data-bs-dismiss="modal">Tutup</button>\s*</div>\s*</div>\s*</div>\s*</div>', content, re.DOTALL)
new_modal = """<!-- MODAL RINGKASAN PENGAJUAN -->
...
if old_info_modal:
    content = content.replace(old_info_modal.group(0), new_modal)
else:
    print("Could not find informasiPerangkatModal")

# 3. Remove old modals for Lihat (lihatFasilitasModal to lihatDokumenModal)
old_lihat_modals = re.search(r'<!-- Modal Lihat Data Fasilitas -->.*?<!-- Modal Tambah Peserta -->', content, re.DOTALL)
if old_lihat_modals:
    content = content.replace(old_lihat_modals.group(0), '<!-- Modal Tambah Peserta -->')
else:
    print("Could not find old lihat modals")

with open('resources/views/user/kegiatan/kegiatan_step2.blade.php', 'w', encoding='utf-8') as f:
    f.write(content)
