...
@php
$mulai = \Carbon\Carbon::parse($pesertaPegawai->tgl_mulai);
$selesai = \Carbon\Carbon::parse($pesertaPegawai->tgl_selesai);
$hariPerjalanan = $selesai->diffInDays($mulai)+1;
        ...
        <table width="550">
            <tbody>
                <tr>
                    ...
                    <td width="200">
                        : {{ \App\Models\Versi::find(session('versi'))->versi ?? '' }} <br>
                        : <br>
                        :
                        {{ $pesertaPegawai->kode_satker }}.{{ $pesertaPegawai->kode_program }}.{{ $pesertaPegawai->kode_kegiatan }}.{{ $pesertaPegawai->kode_output }}.{{ $pesertaPegawai->kode_sub_output }}.{{ $pesertaPegawai->kode_komponen }}.{{ $pesertaPegawai->kode_sub_kegiatan }}.{{ $pesertaPegawai->kode_akun }} <br>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="rincian" width="10">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="rincian">Bandung - {{ $pesertaPegawai->kab_kota }} {{ $pesertaPegawai->provinsi }}
                        <br>
                    ...
                    <td class="rincian">&nbsp;Rp. {{ number_format($pesertaPegawai->nominal_perjadin, 0, ',', '.') }}</td>
                ...
                @php
                $jumlah = $pesertaPegawai->nominal_perjadin;
                ...
                <tr>
                    ...
                    @if ($loop->first) <!-- Menetapkan rowspan hanya pada baris pertama -->
                        <td rowspan="{{ count($groupedFasilitas) }}" class="rincian baris-tiga">
                            <!-- Konten yang Anda inginkan untuk kolom dengan rowspan -->
                        </td>
                    @endif
                </tr>
                @php
...
@php
$mulai = \Carbon\Carbon::parse($pesertaNonPegawai->tgl_mulai);
            ...
            <tbody>
                <tr>
                    ...
                    <td width="200">
                        ...
                        : <br>
                        :
                        {{ $pesertaNonPegawai->kode_satker }}.{{ $pesertaNonPegawai->kode_program }}.{{ $pesertaNonPegawai->kode_kegiatan }}.{{ $pesertaNonPegawai->kode_output }}.{{ $pesertaNonPegawai->kode_sub_output }}.{{ $pesertaNonPegawai->kode_komponen }}.{{ $pesertaNonPegawai->kode_sub_kegiatan }}.{{ $pesertaNonPegawai->kode_akun }} <br>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="rincian" width="10">
            ...
            <tbody>
                <tr>
                    ...
                    <td class="rincian">Bandung - {{ $pesertaNonPegawai->kab_kota }} {{ $pesertaNonPegawai->provinsi }}
                        <br>
                    ...
                    <td class="rincian">&nbsp;Rp. {{ number_format($pesertaNonPegawai->nominal_perjadin, 0, ',', '.') }}</td>
                    <td class="rincian baris-tiga">
                ...
                @php
                $jumlah = $pesertaNonPegawai->nominal_perjadin;

                $groupedFasilitas = [];
                foreach($datas['fasilitasNonPegawais'] as $fasilitasNonPegawai) {
                if($fasilitasNonPegawai->pegawai_id == $pesertaNonPegawai->id) {
                if(!isset($groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan])) {
                $groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan] = [
                'jumlah_frekuensi' => 0,
                'jumlah_harga' => 0,
                'satuan' => $fasilitasNonPegawai->satuan,
                'ket' => ''
                ];
                }
                $groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan]['jumlah_frekuensi'] += $fasilitasNonPegawai->jumlah_frekuensi;
                $groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan]['jumlah_harga'] += $fasilitasNonPegawai->jumlah_harga;

                // Tambahkan jumlah_harga ke total
                $jumlah += $fasilitasNonPegawai->jumlah_harga;

                // Gabungkan 'ket' dengan tanda koma jika tidak kosong
                if (!empty($fasilitasNonPegawai->ket)) {
                if (!empty($groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan]['ket'])) {
                $groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan]['ket'] .= ', ';
                }
                $groupedFasilitas[$fasilitasNonPegawai->nama_kebutuhan]['ket'] .= $fasilitasNonPegawai->ket;
                }
                }
                }
...
@php
$mulai = \Carbon\Carbon::parse($pengemudi->tgl_mulai);
            ...
            <tbody>
                <tr>
                    ...
                    <td width="200">
                        ...
                        : <br>
                        :
                        {{ $pengemudi->kode_satker }}.{{ $pengemudi->kode_program }}.{{ $pengemudi->kode_kegiatan }}.{{ $pengemudi->kode_output }}.{{ $pengemudi->kode_sub_output }}.{{ $pengemudi->kode_komponen }}.{{ $pengemudi->kode_sub_kegiatan }}.{{ $pengemudi->kode_akun }} <br>
                    </td>
                </tr>
            </tbody>
        </table>
        ...
        <table class="rincian" width="10">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="rincian">Bandung - {{ $pengemudi->kab_kota }} {{ $pengemudi->provinsi }}
                        <br>
                    ...
                    <td class="rincian">&nbsp;Rp. {{ number_format($pengemudi->nominal_perjadin, 0, ',', '.') }}</td>
                    <td class="rincian baris-tiga">
                ...
                @php
                $jumlah = $pengemudi->nominal_perjadin;
                ...
                <tr>
                    ...
                    <td class="rincian"><b>Jumlah</b></td>
                    <td class="rincian">&nbsp;<b>Rp. <?php echo number_format($jumlah, 0, ',', '.'); ?></b></td>
                    <td class="rincian" style="border-top: none;"></td>
                </tr>
                <tr>
                    <td class="rincian"></td>
                    <td class="rincian" colspan="3">Terbilang : <?php echo ucwords($terbilang); ?> Rupiah</td>
